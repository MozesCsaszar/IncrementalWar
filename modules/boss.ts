...
class BossArmySelectionPageClass extends PageClass {
    ...
    initializeEventListeners() {
        ...
        this.startFightButton.addEventListener('click', function () {
            ...
            BossFightingPage.fight = obj.fight;
            for (let i = 0; i < obj.fight.max_selectible_armies; i++) {
                if (obj.fight.selected_armies[i] != -1) {
                    let result = TowerPage.Tower.removeRaidedLevelByArmy(obj.fight.selected_armies[i]);
                    if (result != undefined) {
...
class BossFightingPageClass extends PageClass {
    ...
    displayEveryTick(obj) {
        //fill in sliders
        for (let i = 0; i < obj.fightingArmies.length; i++) {
            //health foreground
            obj.armyStatusBars[i][0].style.width = obj.get_width(obj.fightingArmies[i].total_health, obj.fightingArmies[i].max_total_health);
            obj.armyStatusBars[i][1].innerHTML = StylizeDecimals(obj.fightingArmies[i].total_health) + '/' + StylizeDecimals(obj.fightingArmies[i].max_total_health);
            //unit nr foreground
            obj.armyStatusBars[i][2].style.width = obj.get_width(obj.fightingArmies[i].units, obj.fightingArmies[i].max_units);
            obj.armyStatusBars[i][3].innerHTML = StylizeDecimals(obj.fightingArmies[i].units, true) + '/' + StylizeDecimals(obj.fightingArmies[i].max_units, true) +
                ' (' + StylizeDecimals(obj.fightingArmies[i].deployed, true) + ')';
            //attack status foreground
            obj.armyStatusBars[i][4].style.width = obj.get_width(obj.fightingArmies[i].attack_counter, obj.fightingArmies[i].attack_time);
            obj.armyStatusBars[i][5].innerHTML = StylizeDecimals(obj.fightingArmies[i].get_total_attack(obj.fightingBosses[0]));
        }

        for (let i = 0; i < obj.fightingBosses.length; i++) {
            //health foreground
            obj.bossStatusBars[i][0].style.width = obj.get_width(obj.fightingBosses[i].total_health, obj.fightingBosses[i].max_total_health);
            obj.bossStatusBars[i][1].innerHTML = StylizeDecimals(obj.fightingBosses[i].total_health) + '/' + StylizeDecimals(obj.fightingBosses[i].max_total_health);
            //unit nr foreground
            obj.bossStatusBars[i][2].style.width = obj.get_width(obj.fightingBosses[i].units, obj.fightingBosses[i].max_units);
            obj.bossStatusBars[i][3].innerHTML = StylizeDecimals(obj.fightingBosses[i].units, true) + '/' + StylizeDecimals(obj.fightingBosses[i].max_units, true);
            //attack status foreground
            obj.bossStatusBars[i][4].style.width = obj.get_width(obj.fightingBosses[i].attack_counter, obj.fightingBosses[i].attack_time);
            obj.bossStatusBars[i][5].innerHTML = StylizeDecimals(obj.fightingBosses[i].get_total_attack());
        }
